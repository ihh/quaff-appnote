...
\abstract{
  Nanopore sequencing generates high volume long reads with a relatively high error-rate,
  motivating the development of an error-tolerant read alignment tool that can be easily parallelized.
  Quaff is an open source command-line tool for aligning long FASTQ-format reads to a FASTA reference genome using a conditional pair HMM (transducer) that also models quality scores.
